...
public class PendingOrderModel implements PendingOrderContractor.Model {


    private PendingOrderPresenter mPresenter;
    private CompositeDisposable disposable = new CompositeDisposable();
    private AppRepository appRepository;
    private ApiInterface apiInterface;

    PendingOrderModel(PendingOrderPresenter presenter, AppRepository appRepository1) {
    ...
    public void requestCancelOrder(int position, String orderId, String comment) {
        ...
        cancelOrderRequest.setReason(comment);
        cancelOrderRequest.setLang(appRepository.getLanguageCode());
        disposable.add(apiInterface
                .cancelOrderDetails(cancelOrderRequest)
                ...
                .subscribeWith(new DisposableSingleObserver<BaseResponse<OrderDetailResponse>>() {
                    ...
                    public void onSuccess(BaseResponse<OrderDetailResponse> baseResponse) {
                        if (baseResponse.getCode() == 200) {
                            mPresenter.cancelOrderResponse(position, baseResponse.getMessage());
                        } else {
