...
public class YourAddressModel implements YourAddressContractor.Model {


    private YourAddressPresenter mPresenter;
    private CompositeDisposable disposable = new CompositeDisposable();
    private AppRepository appRepository;
    private ApiInterface apiInterface;

    YourAddressModel(YourAddressPresenter presenter, AppRepository appRepository1) {
                ...
                .subscribeWith(new DisposableSingleObserver<BaseResponse<OtpResponse>>() {
                    ...
                    public void onSuccess(BaseResponse<OtpResponse> baseResponse) {
                        if (baseResponse.getCode() == 200) {
                            appRepository.setLatitude(latitude);
                            appRepository.setLongitude(longitude);
                            mPresenter.onSuccess(baseResponse.getMessage(),postalAddress);
                        }else if(baseResponse.getCode()==201){
                             mPresenter.otpRequest(String.valueOf(baseResponse.getData().getOtp()),baseResponse.getMessage());
                        }else if(baseResponse.getCode()==400 && baseResponse.getMessage().equals(VERIFICATION_CHECK)){
                            mPresenter.mobileEmailError(baseResponse.getMessage());
                        } else {
        ...
        disposable.add(apiInterface
                ...
                .subscribeWith(new DisposableSingleObserver<BaseResponse>() {
                    ...
                    public void onSuccess(BaseResponse baseResponse) {
                        if (baseResponse.getCode() == 200) {
                            mPresenter.onSuccess(baseResponse.getMessage(),shippingAddressRequest.getSh_location());
                        } else {
                            mPresenter.apiError(baseResponse.getMessage());
                        }
                    }
