...
public class HelpModel implements HelpContractor.Model {
    ...
    HelpModel(HelpPresenter presenter, AppRepository appRepository1) {
    ...
    public void requestHelpContent() {
        ...
        disposable.add(apiInterface
                .getHelpContent(languageRequest)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new DisposableSingleObserver<BaseResponse<Support>>() {
                    @Override
                    public void onSuccess(BaseResponse<Support> baseResponse) {
                        if (baseResponse.getCode() == 200) {
                            mPresenter.onContent(baseResponse.getData());
                        } else {
    ...
    public void requestPrivacyPolicyContent() {
        ...
        languageRequest.setLang(appRepository.getLanguageCode());
        disposable.add(apiInterface
                .getPrivacyContent(languageRequest)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new DisposableSingleObserver<BaseResponse<Support>>() {
                    @Override
