...
public class ReviewItemAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ReviewItemAdapter(Context context, List<ItemReviewList> itemReviewLists) {
        this.itemReviewLists = itemReviewLists;
        this.mContext = context;
    }

    public void setReviewItemListener(ReviewItemClickListener refundDetailsListener) {
        this.refundDetailsListener = refundDetailsListener;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ItemReviewList itemReviewList = itemReviewLists.get(position);
        switch (getItemViewType(position)) {
            case ITEM:
                ...
                GlideUtils.showImage(mContext, itemVH.ivFood, R.drawable.image_placeholder_small, itemReviewList.getItemImage());
                holder.itemView.setOnClickListener(v -> refundDetailsListener.clickItem(position));
                itemVH.tvStatus.setText(itemReviewList.getReviewStatus()==0?mContext.getString(R.string.disapproved):mContext.getString(R.string.approved));
                itemVH.tvStatus.setTextColor(itemReviewList.getReviewStatus()==0? Color.RED:Color.GREEN);
                break;
            case LOADING:
//                Do nothing
                break;
    ...
    public void add(ItemReviewList r) {
        itemReviewLists.add(r);
        notifyItemInserted(itemReviewLists.size() - 1);
    }

    public void addAll(List<ItemReviewList> restautrantDetails) {
        for (ItemReviewList result : restautrantDetails) {
            add(result);
        }
    }
    ...
    public void removeLoadingFooter() {
        ...
        ItemReviewList result = getItem(position);
    ...
    private ItemReviewList getItem(int position) {
