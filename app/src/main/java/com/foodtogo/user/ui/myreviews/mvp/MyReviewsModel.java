...
public class MyReviewsModel implements MyReviewsContractor.Model {
    ...
    MyReviewsModel(MyReviewsPresenter presenter, AppRepository appRepository1) {
    ...
    public void requestMyReviews(int pageNo) {
        MyReviewRequest myReviewRequest = new MyReviewRequest();
        myReviewRequest.setLang(appRepository.getLanguageCode());
        myReviewRequest.setPage_no(String.valueOf(pageNo));
        disposable.add(apiInterface
                .requestMyReviews(myReviewRequest)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new DisposableSingleObserver<BaseResponse<MyReviewResponse>>() {
                    @Override
                    public void onSuccess(BaseResponse<MyReviewResponse> baseResponse) {
                        if (baseResponse.getCode() == 200) {
                            if (pageNo == 1) {
                                mPresenter.onMyReviews(baseResponse.getData());
                            } else {
