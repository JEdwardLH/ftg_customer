...
public class GeoCodingRepository implements IGeoCodingRepository {
    ...
    @Override
    public Observable<String> getObservableAddressStringFromLatLng(final double latitude, final double longitude, final String apiKey,final String language) {
        return Observable.create(new ObservableOnSubscribe<String>() {
            @Override
            public void subscribe(final ObservableEmitter<String> source) throws Exception {
                String latLng = latitude + "," + longitude;
                Log.d("dsadsadas",apiKey);
                geocodeAPI.getPolylineData(latLng, apiKey,language).enqueue(new Callback<JsonObject>() {
                    @Override
                                    ...
                                    if (status.equals(NiboConstants.OK)) {
                                        ...
                                        if (jsonArray.length() == 0) {
                                            ...
                                            for (int index = 0; index < addressComponents.length(); index++) {
                                                JSONObject jsonObject = addressComponents.getJSONObject(index);
                                                JSONArray types = jsonObject.getJSONArray("types");
                                                if (types.toString().contains("locality")) {
                                                    AddressConstants.CITY = jsonObject.getString("long_name");
                                                } else if (types.toString().contains("administrative_area_level_1")) {
                                                    AddressConstants.STATE = jsonObject.getString("long_name");
                                                } else if (types.toString().contains("country")) {
                                                    AddressConstants.COUNTRY = jsonObject.getString("long_name");
                                                } else if (types.toString().contains("postal_code")) {
                                                    AddressConstants.PIN_CODE = jsonObject.getString("long_name");
                                                }
                                            }
