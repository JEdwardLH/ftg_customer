...
public class DirectionsRepository implements IDirectionsRepository {
    ...
    @Override
    public Observable<List<Route>> getRouteForPolyline(final String origin, final String destination, final String apiKey,final String lang) {
        return Observable.create(new ObservableOnSubscribe<List<Route>>() {
            ...
            public void subscribe(final ObservableEmitter<List<Route>> source) throws Exception {
                directionsApi.getPolylineData(String.valueOf(origin), String.valueOf(destination), apiKey,lang).enqueue(new Callback<JsonObject>() {
                    @Override
